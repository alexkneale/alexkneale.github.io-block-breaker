...
export class Block {
    ...
    public static blockCentreDistance = (
        block1: Block,
        block2: Block
    ): number => {
        return Math.sqrt(
            (block1.x - block2.x) ** 2 + (block1.y - block2.y) ** 2
        );
    };

    // function for checking two blocks are overlapping
    public static isTooClose = (
        firstBlock: Pair,
        secondBlock: Pair,
        width: number,
        height: number
    ...
    public static generateFilteredRandomArray = (
        ...
        while (result.length < n && attempts < maxAttempts) {
            ...
            const candidate: Pair = [
                Block.getRandomInRange(width, canvasWidth - width),
                Block.getRandomInRange(0, 0.75 * canvasHeight),
            ];

            // check for overlap before pushing
            const isValid = result.every(
                (existing) =>
                    !Block.isTooClose(candidate, existing, width, height)
            );
